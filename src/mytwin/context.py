...
# Cache the prompt globally once it is generated so we don't hit the disk constantly
TWIN_SYSTEM_PROMPT = None

def get_twin_system_prompt():
    global TWIN_SYSTEM_PROMPT
    if TWIN_SYSTEM_PROMPT is not None:
    ...
    You have access to a tool named `query_additional_knowledge`. If a visitor asks a specific question about your career 
    that isn't instantly clear in the # Resume context, you MUST call `query_additional_knowledge` ONCE to check if a valid answer 
    has been recorded there before stating that you do not know the answer.
    ...
    IMPORTANT:
    If you don't know the answer, use your tool to record the question and your reply MUST include 'do not have details'.
    Ask if they would like to contact the person directly. 
